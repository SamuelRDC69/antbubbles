...
async function maybeReadRedis(req: NextRequest) {
  ...
  try {
    const cached = await redis.get<Candle[]>(REDIS_KEYS.offchainChart('taco', tokenId, resolution))
    ...
    const data = await buildUsdChartDataFromRawCandles('taco', {
      pair: req.nextUrl.searchParams.get('pair'),
      path: req.nextUrl.searchParams.get('path'),
      symbol: req.nextUrl.searchParams.get('symbol'),
      resolution: req.nextUrl.searchParams.get('resolution'),
...
export async function GET(req: NextRequest) {
  ...
  const out = await buildOffchainChartData('taco', {
    pair: req.nextUrl.searchParams.get('pair'),
    path: req.nextUrl.searchParams.get('path'),
    symbol: req.nextUrl.searchParams.get('symbol'),
    resolution: req.nextUrl.searchParams.get('resolution'),
  ...
  return NextResponse.json(out, {
    headers: { 'Cache-Control': 'no-store' },   // always fresh — data grows in real-time
  })
}
