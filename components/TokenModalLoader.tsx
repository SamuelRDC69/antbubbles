...
export default function TokenModalLoader({
  ...
  const { t } = useI18n()
  const translatedLabel = label ?? t('loadingTokenMarket')
  return (
    <div className="fixed inset-0 z-50 flex items-end overflow-hidden sm:items-center sm:p-4">
      ...
      >
        ...
        <div className="flex flex-col items-center gap-4">
          <LiquidLoader label={translatedLabel} size="large" />
          <span className="text-sm font-medium text-gray-400">{translatedLabel}…</span>
        </div>
      </div>
    </div>
  )
